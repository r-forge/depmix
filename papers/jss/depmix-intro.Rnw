...
\section{Known issues and future plans}

\paragraph{Constraint violations} Parameter optimization is done by adding
a penalty to the log likelihood whenever constraints are not satisfied, ie
linear inequality and box constraints.  Equality constraints are fitted by
reparametrization and do not suffer from this problem.  Refer to the
section on parameter estimation and the \code{fitdmm} help page on how to
deal with constraint violations. Future plans include using Lagrange
multipliers to overcome this problem. 

\nocite{Raijmakers2001}
\nocite{Maas2005b}
\nocite{Schmittmann2006}
